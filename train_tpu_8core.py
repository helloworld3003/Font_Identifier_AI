...
def _mp_fn(index, flags):
    ...
    for epoch in range(start_epoch, MAX_EPOCHS + 1):
        ...
        for relative_batch_idx, (images, labels) in enumerate(mp_device_loader):
            ...
            # PyTorch XLA CRASH FIX: We cannot use xm.all_gather on the embeddings before loss.backward() 
            # because the XLA autograd engine does not have a registered kernel for it, which causes
            # a massive C++ Segfault. We will compute ArcFace strictly on the local batch, which is
            # still incredibly difficult and forces deep feature learning!
